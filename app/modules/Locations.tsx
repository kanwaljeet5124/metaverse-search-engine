...
export default function Locations() {
    ...
    const {data, isLoading, error} = useLocationSearch(keyword, page as number);
                ...
                {!isLoading && data?.status && data.results.map((item, index) => <Card key={item.name + index} className="max-w-[400px]">
                    <CardHeader className="flex gap-3">
                        <div className="flex flex-col">
                            ...
                            <p className="text-small text-default-500">{item.type}</p>
                        </div>
                    </CardHeader>
                    <Divider />
                    <CardBody>
                        <p className="text-sm">There are {item.residents.length} permanant residents on this locations.</p>
                    </CardBody>
                    <Divider />
                    <CardFooter>
                        <p className="text-sm"><b>Dimension: </b>{item.dimension}</p>
                    </CardFooter>
                </Card>)}
